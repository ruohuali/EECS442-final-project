# -*- coding: utf-8 -*-
"""Copy_of_Starter_EECS442_FA21_PS7_Object_Detection (1).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1DlyzhEZcDfTzPrhatf84M9BW5OX6A4eg

#EECS 442 Fall 2021 PS7: Object Detection
...
In this problem set, you will be implementing parts of a single stage object detector closely related to [`YOLOv1`](https://arxiv.org/pdf/1506.02640.pdf). We will also evaluate the detection accuracy using the classic metric mean Average Precision ([`mAP`](https://github.com/Cartucho/mAP)).

Feel free to take a look at the paper linked above, or this comprehensive [`summary`](https://towardsdatascience.com/yolov1-you-only-look-once-object-detection-e1f3ffec8a89) of the paper by Sik-Ho Tsang.

*Wait a minute! Before you begin, check out this cool video of the [YOLOv1 object detector watching sports](https://youtu.be/Gl1rxvEvgEs).*
...
def SingleDetectionInference(detector, img, idx_to_class, thresh=0.8, nms_thresh=0.05):
    ...
    batch_size = len(images)
    for idx in range(batch_size):
        torch.clamp_(final_proposals[idx][:, 0::2], min=0, max=600)
        torch.clamp_(final_proposals[idx][:, 1::2], min=0, max=400)

        # valid_box = sum([1 if j != -1 else 0 for j in boxes[:, 0]])
        final_all = torch.cat((final_proposals[idx], \
                               final_class[idx].float(), final_conf_scores[idx]), dim=-1).cpu()
        resized_proposals = coord_trans(final_all, torch.tensor([600]), torch.tensor([400]))
